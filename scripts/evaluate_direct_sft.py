...
def test_direct_sft_model(config: AppConfig):
    ...
    base_model.model = PeftModel.from_pretrained(base_model.model, f"{config.directsft.output_dir}").to(base_model.model.device)
    ...
    # preds_answer = extract_generated_answer(generated_answers)
    ...
    metrics_path = "metrics/directsft_metrics_eval.json"
